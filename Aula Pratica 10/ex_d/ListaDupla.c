...
int troca(Lista *l, int pos1, int pos2)
{
    ...
    if (listaVazia(l) == 0)
        return 2;
    if (pos1 > tamanho(l) || pos2 > tamanho(l))
        return 3;
    if (pos1 == pos2)
        return 4;
    No *n1 = l->inicio;
...
int aluno_rep(Lista *l1, Lista *l2, Aluno it)
{
    if (l1 == NULL || l2 == NULL)
        return 1;
    ...
    No *n1 = l1->inicio;
    No *n2 = l2->inicio;
    int teste = 1, teste2 = 1;
    while (n1 != NULL)
    {
        if (n1->valor.mat == it.mat)
            teste = 0;
        n1 = n1->prox;
    }
    while (n2 != NULL)
    {
        if (n2->valor.mat == it.mat)
            teste2 = 0;
        n2 = n2->prox;
    }
    if (teste2 == 0 && teste == 0)
...
int removerRepetido(Lista* l) {
    ...
    while (atual->prox != NULL) {
        ...
        while (comparar->prox != NULL) {
            if (atual->valor.mat == comparar->valor.mat) {
                No* temp = comparar;
                if (comparar->prox == NULL) {
                    comparar->ant->prox = NULL;
                    free(temp->prox);
                }else{
                    comparar->ant->prox = comparar->prox;
                    free(temp);
                }    
            }
